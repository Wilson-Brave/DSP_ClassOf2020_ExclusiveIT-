...
service storageSystem on ep {
    ...
    resource function addRecord(grpc:Caller caller, clientRqRecord value) {
        ...
        json[] readed2 = <json[]>readfile;
        alljson[0] = readed2;
    
        var wResult = write(alljson, filePath);
        if (wResult is error) {
        ...
        var readfile1 = read(filePath); //convert to json to check if result
    }
    ...
    resource function updateRecord(grpc:Caller caller, RecUpdate value) {
        ...
        // byte [] cryptovalue = crypto:hashMd5(bytevalue);
        // value.hashkey = cryptovalue.toBase16();
        value.hashkey = "9d736f43f4e8d38f616ae114b4aefede";
        value.ver = "1.1";
    ...
    resource function readRecord(grpc:Caller caller, RecReadKey value) {
        ...
        alljson[1] = filefinal;
            ...
            //////////////////////////////////////
            //First Method - Client passes key only and server returns Record
            if (value.hashkey != filefinal.hashkey) {
                io:print("Record does not exist!");
            }
            var readresult = caller->send(alljson.toJsonString());
            ////////////////////////////////////
            //Implementation of Second and Third options of reading not understood completely
    }
